...
// WHERE I LEFT THINGS: Correctly draws hexagon grid, but you can't place any lines yet
...
int[] findNearest(int x, int y) {
  int row = (x+RADIUS/2)%(RADIUS*3/2);
  int col = floor(y%(RADIUS*SQRT3));
  row = x - row+RADIUS/2;
  col = floor(y - col+RADIUS*SQRT3/2);
  if (row%RADIUS==0)col = floor(col - RADIUS*SQRT3/2);
